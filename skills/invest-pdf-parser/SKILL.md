...
| invest-lark-cli 阶段 2 批量转换 | lark-sync 调用（输出到 staging） |
...
由 `invest-doc-router` 触发时，源 PDF 位于 `source/` 子目录，解析后的 `.md` 输出到**父目录**（即 doc_type 目录）：

```
data/companies/tme/sellside/briefings/source/tme_4q25.pdf   ← 源文件（输入）
data/companies/tme/sellside/briefings/tme_4q25.md            ← 解析结果（输出）
```
...
由用户直接触发时（非 doc-router 流程），在原 PDF 同目录下生成同名 `.md` 文件：
```
